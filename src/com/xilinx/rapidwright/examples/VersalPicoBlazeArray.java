...
public class VersalPicoBlazeArray {
	...
	private static final int BRAMS_IN_CLOCK_REGION_HEIGHT = 24;
	...
	public static void updateAnchorToBRAM(Module m){
		for(SiteInst i : m.getSiteInsts()){
			if(i.getSite().getSiteTypeEnum() == SiteTypeEnum.RAMB36){
				//System.out.println("Setting anchor to " + i);
				m.setAnchor(i);
	...
	public static void main(String[] args) {
		...
		//System.out.println("Creating design with part: " + args[1] + " in " + System.getProperty("user.dir"));
		...
		for(int i=0; i < implementationCount; i++){
			...
			picoBlazeImpls[i] = new Module(d, metaName);
		...
		TileColumnPattern bramPatternRight = TileColumnPattern.createTileColumnPattern(Arrays.asList(TileTypeEnum.BRAM_ROCF_TR_TILE));
		TileColumnPattern bramPatternLeft = TileColumnPattern.createTileColumnPattern(Arrays.asList(TileTypeEnum.BRAM_ROCF_TL_TILE));
		int bramColumns = TileColumnPattern.genColumnPatternMap(device).get(bramPatternRight).size();
		bramColumns += TileColumnPattern.genColumnPatternMap(device).get(bramPatternLeft).size();
