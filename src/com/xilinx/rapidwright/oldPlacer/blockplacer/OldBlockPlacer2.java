...
public class OldBlockPlacer2 {
	...
	private void initializePlacer(boolean debugFlow){
		...
		currentMove = new OldMove();
		totalMoves = 0;
		dev = design.getDevice();
		allPaths = new HashSet<OldPath>();
		hardMacros = new ArrayList<OldHardMacro>();
		macroMap = new HashMap<ModuleInst, OldHardMacro>();
		...
		for(ModuleInst mi : design.getModuleInsts()){
			OldHardMacro hm = new OldHardMacro(mi);
			hardMacros.add(hm);
	...
	private void initialPlacement(){
		currentPlacements = new HashMap<Site, OldHardMacro>();
		...
		// Place hard macros for initial placement
		for(OldHardMacro hm : hardMacros){
			sites.clear();
		...
		// We have p
		for(OldPath path : allPaths){
			path.calculateLength();
		}
		
		ArrayList<OldHardMacro> prunedList = new ArrayList<>();
		for(OldHardMacro hm : new ArrayList<>(hardMacros)){
	...
	private void populateAllPaths(){
		...
		for(OldPath pa : allPaths){
			for(OldPathPort po : pa){
				if(po.getBlock() != null){
	...
	private boolean checkValidPlacement(OldHardMacro hm){
		if(!hm.isValidPlacement()) return false;
		for(OldHardMacro hardMacro : hardMacros){
	...
	public double calculateStartTemp(int numBlocks){
		...
		for(int i=0; i<numBlocks; i++){
			OldHardMacro selectedHD = hardMacros.get(i);
	...
	public Design placeDesign(Design design, boolean debugFlow){
		...
		OUTER: while(!finished){
			...
			//for(int inner_iterate = 0; inner_iterate< (dev.getColumns()*dev.getRows()); inner_iterate++){
				OldHardMacro selectedHD = hardMacros.get(rand.nextInt(hardMacros.size()-1));
				if (getNextMove(selectedHD)){
					totalMoves++;
					...
					if(currentMove.getBlock0() != null){
						for(OldPath wire : currentMove.getBlock0().getConnectedPaths()){
					...
					if(currentMove.getBlock1() != null){
						for(OldPath wire : currentMove.getBlock1().getConnectedPaths()){
							wire.calculateLength();
							tmp = tmp + wire.getLength();
						}
		...
		*/
		...
		OldHardMacro[] array = new OldHardMacro[hardMacros.size()];
	...
	private boolean getNextMove(OldHardMacro selected){
		...
		Site site1 = null;
		OldHardMacro hm0 = selected;
		OldHardMacro hm1 = null;
		int iterations = 0;
		/*int minX = 0;
		int maxX = 0;
		int minY = 0;
